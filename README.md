...
Клавиши: `enter` — детали, `o` — браузер, `y` — скопировать ключ задачи (задачи и работы — списки
и карточки), `Y` — меню копирования (задачи, работы, PR), `R` — синк всего, `c` — свернуть панель
изменений, `q` — выход (работает и из модалки копирования).
